...
async fn do_clone(op: &GitOperation, policy: &GaitPolicy) -> Result<String> {
    ...
    // Create parent directories only if the *parent* is inside trusted paths.
    // We canonicalize the parent (which must exist or be creatable) to check.
    ...
    let canonical_target = if target.exists() {
        ...
        // Target doesn't exist yet — canonicalize parent + last component.
        let parent = target
            .parent()
            .ok_or_else(|| anyhow::anyhow!("invalid target_dir"))?;
        let canonical_parent = std::fs::canonicalize(parent)?;
        let last = target
            .file_name()
            .ok_or_else(|| anyhow::anyhow!("invalid target_dir"))?;
        canonical_parent.join(last)
    };

    // SECURITY: strict prefix check — clone target MUST be inside trusted paths.
